...
export default function RegistroVoto() {
  ...
  const handleSubmit = async (e) => {
    ...
    setMensaje(res.ok ? `✅ ${data.message}` : `❌ ${data.error || 'Error inesperado'}`);
  };
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <select value={idEleccion} onChange={e => setIdEleccion(e.target.value)} className="border p-2" required>
        ...
        <select value={numeroLista} onChange={e => setNumeroLista(e.target.value)} className="border p-2" required>
